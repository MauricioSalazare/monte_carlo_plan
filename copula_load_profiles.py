...
def power_factor_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    pf_columns = []
    pf_values = []
    for ii in time_step_number:
        pf_columns.append(f"pf_{ii}")
        pf_values.append(data[[f"q_{ii}_ap",
                               f"q_{ii}_rp"]].apply(lambda x: cos_phi(x[f"q_{ii}_ap"], x[f"q_{ii}_rp"]), axis=1))
...
def compute_quantiles(dataset: np.ndarray) -> (np.ndarray, np.ndarray, np.ndarray):
    ...
    q_05 = np.nanquantile(dataset, q=0.05, axis=1)
    q_50 = np.nanquantile(dataset, q=0.50, axis=1)
    q_95 = np.nanquantile(dataset, q=0.95, axis=1)

    return (q_05, q_50, q_95)

#%%
def sampling_by_annual_energy(copula_model: EllipticalCopula,
                              data_copula: pd.DataFrame,
                              *,
    ...
    ----------
        ...
        data_copula: pd.DataFrame: Original pivoted data set. Dimensions (n x p): n: days p: variables.
    ...
    var_mapping = dict(zip(data_copula.columns,
                           [f"x{ii + 1}" for ii in range(len(data_copula.columns))]))
    cond_variable = var_mapping["avg_gwh"]

    if energy_values is None:
        energy_values_sample = data_copula.loc[:, "avg_gwh"].value_counts()
    else:
        energy_values_sample = energy_values.copy()

    avg_energy_list = []
    samples_list = []
    for energy_, count_ in tqdm(energy_values_sample.iteritems(), total=energy_values_sample.shape[0]):
        samples_list.append(copula_model.sample(count_, conditional=True, variables={cond_variable: energy_}, drop_inf=True))
    ...
    if energy_values is None:
        samples_frame = pd.DataFrame(np.concatenate(samples_list, axis=1).T,
                                     index=data_copula.index,
                                     columns=data_copula.columns[:-1])
        samples_cluster_frame = pd.concat([samples_frame, data_copula.loc[:, "avg_gwh"]], axis=1)

    else:
        samples_frame = pd.DataFrame(np.concatenate(samples_list, axis=1).T,
                                     columns=data_copula.columns[:-1])
        samples_cluster_frame = pd.concat([samples_frame, avg_energy_frame], axis=1)

    return samples_cluster_frame
...
for cluster_ in range(3):
    ...
    pf_values_original = power_factor_data(data_copula)
    pf_values_samples = power_factor_data(samples_cluster_frame)
...
plt.savefig('figures/load_model/sampling_load_models.png', dpi=700, bbox_inches='tight')

#%% Create samples conditioned by yearly energy consumption.
CLUSTER = 2
LOW_ANNUAL_ENERGY = 0.429440
# LOW_ANNUAL_ENERGY = 0.03
HIGH_ANNUAL_ENERGY = 1.767857
annual_energy_to_sample = pd.Series({HIGH_ANNUAL_ENERGY: 100, LOW_ANNUAL_ENERGY: 100}, name="avg_gwh")  # Dict[energy_level: samples]
samples_copula_cluster_energy = sampling_by_annual_energy(copula_model=copula_models[CLUSTER]["copula"],
                                                          data_copula=copula_models[CLUSTER]["original_data"],
...
for ii, regex_dict in zip(range(2), filter_dict):
    ...
    _, q_50_low, _ = compute_quantiles(samples_copula_cluster_energy.iloc[idx_low, :].filter(**regex_dict).values.transpose())
    _, q_50_high, _ = compute_quantiles(samples_copula_cluster_energy.iloc[idx_high, :].filter(**regex_dict).values.transpose())
...
plt.savefig('figures/load_model/sampling_conditioned.png', dpi=700, bbox_inches='tight')