...
ax[1].plot(x_axis, day_mapping[DAY_TO_PLOT]["q_t_all"], linewidth=1.4, color="green", marker=".", markersize=0.3)
...
# ax[0].plot(profile_clear[CLEAR_DAY], color="red", label="Clear day")
# ax_2 = ax[0].twiny()
# ax_2.plot(profile_cloudy[CLOUDY_DAY], color="blue", label="Cloudy day")
#
# ax_2.set_xlabel("Time stamp - Cloudy day")
# ax[0].set_xlabel("Time stamp - Clear day")
# ax[0].set_ylim(0, 1000)
# ax[0].set_ylabel("$W/$m$^2$")
# ax[0].legend(fontsize="xx-small")
# ax_2.legend(fontsize="xx-small")
#
# ax[1].scatter(x_t_clear, y_t_clear, color="red", marker="x", s=10, label="FD Clear day")
# ax[1].plot(x_t_clear, rg_fd_clear.intercept + rg_fd_clear.slope * x_t_clear, color='red', linewidth=0.4, label='Fitted line clear')
#
# ax[1].scatter(x_t_cloudy, y_t_cloudy, color="blue", marker="o", s=10, label="FD Cloudy day")
...
profiles_sampled = []
pivoted_series_sampled = []

for pi_mixture_ in tqdm(pi_mixtures):
    profile_series, pivoted_series = generate_profiles(pi_mixture_, mvt_models, day_mapping, data_aligned_sliced, START_DATE, END_DATE)
    profile_series_concat = pd.concat([profile_series, data_aligned_sliced[["ghi_haurwitz", "qg"]]], axis=1)
