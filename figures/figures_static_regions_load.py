...
def plot_quantile_mixture_case_loading(solution_quantile,
                                       ax,
                                       max_current,
...
def get_solutions_dictionary(path_file_solutions, quantiles: List[str], line_files=False) -> dict:
    """
...
#%%
critical_quantiles = ["90", "95", "75", "50"]
path_file_parent = Path(r"D:\monte_carlo_solutions_AWS_quantiles_loading")
solutions_dictx_new = get_solutions_dictionary(path_file_parent, quantiles=critical_quantiles, line_files=True)

# Re-arrange dictionary, so the code in the first figure can work
cases_combinations = list(set(solutions_dictx_new["95"].keys()))
...
list_matrices_plot = []
list_of_tuple_cases = []
matrix_loading = np.zeros((len(x), len(y)))
for mixture_case in mixture_cases_plot:
    for i, pv in enumerate(x):
        for j, load in enumerate(y):
            case = (mixture_case, load, pv)
            list_of_tuple_cases.append(case)
            matrix_loading[i, j] = np.max(solutions_dict[case]["max_q_" + QUANTILE])
    list_matrices_plot.append(matrix_loading.copy())
warnings.filterwarnings("default")  # This is dangerous as plotting issues are not raised
...
gs0 = gridspec.GridSpec(2, 1, figure=fig, wspace=0.2, hspace=0.08, left=0.11, bottom=0.11, right=0.98, top=0.93,
                        height_ratios=[1, 1.6], width_ratios=[1])  # Two columns to hold the different plots
gs00 = gs0[0].subgridspec(1, 3, wspace=0.35)  # Colormap
gs01 = gs0[1].subgridspec(3, 3, wspace=0.35, hspace=0.08)  # Voltage profiles
...
min_load_cbar, max_load_cbar = 140 / I_MAX , 474 / I_MAX

max_technical_load_green = float(I_MAX / I_MAX)
max_technical_load_caution = np.round(max_technical_load_green * 1.16, 3)
max_technical_load_danger = np.round(max_technical_load_green * 1.16, 3)
...
for ii, (ax_0, ax_1, ax_2, ax_3, mixture_case) in enumerate(zip(ax_0_row,
    ...
    data_mixture_case = [solutions_dict[(mixture_case, 1.0, 0.0)] ,
    ...
    for data_mixture_ in data_mixture_case:
        coords_max.append(get_coords_max_values(data_mixture_["max_q_" + QUANTILE] / I_MAX))

    colors_ = ["g", "b", "purple"]

    ax_0.pcolor(x, y, list_matrices_plot[ii] / I_MAX, shading='auto', vmin=min_load_cbar, vmax=max_load_cbar)
    ...
    cbar.ax.set_xlabel("Max. Transf. Loading Rate", fontsize="large")
    ...
    rect_contour = patches.Rectangle(special_rectangles[ii], width=0.1, height=0.1, linewidth=2, edgecolor='purple',
                                     facecolor='none', linestyle="-")
    rect_set = [rect_low_left, rect_low_right, rect_contour]
    ...
    ax_row = [ax_1, ax_2, ax_3]  # Rows

    for ax_col, color_, data_mixture_, (x_max_, y_max_) in zip(ax_row, colors_, data_mixture_case, coords_max):
        plot_quantile_mixture_case_loading(data_mixture_,
                                           ax=ax_col,
                                           max_current=I_MAX,
    ...
    if ii == 0:  # ii Stands for columns in the plot
        ...
        ax_1.set_ylabel(f"Load: 1.0\nPV: 0.0" + "\nLoading rate", fontsize="large")
        ax_2.set_ylabel(f"Load: 0.0\nPV: 0.0" + "\nLoading rate", fontsize="large")
        # ax_3.set_ylabel(f"Load: 0.5\nPV: 0.5" + "\nVoltage [p.u.]", fontsize="large")
        ax_3.set_ylabel(f"Loading rate", fontsize="large")

        leg = ax_3.legend(fontsize="large",
                          bbox_to_anchor=(0.35, -0.25),
                          loc="upper left",
                          ncol=5,
                          title="Max. loading percentiles",
    ...
    ax_1.set_ylim((20 / I_MAX, 350 / I_MAX))
    ax_2.set_ylim((0, 300 / I_MAX))
    ax_3.set_ylim((30 / I_MAX, 310 / I_MAX))

plt.savefig('static_regions/static_region_loading_border.pdf', dpi=700, bbox_inches='tight')
...
titles_list = ["(a)", "(b)", "(c)"]
critical_quantiles = list(set(critical_quantile_max))
solutions_dictx = get_solutions_dictionary(path_file_parent, quantiles=critical_quantiles, line_files=True)
...
for ii, ax in enumerate(ax_t):  # Iterate through columns
    ...
    list_matrices = get_matrices_critical_quantiles(quant_to_process_max,
                                                    ...
                                                    load_growth_percentiles=y,
                                                    process_max=True)
    list_matrices = [matrix / I_MAX for matrix in list_matrices]
    q_05_max, q_50_max, q_95_max, _ = plot_contour_levels_irradiance_days(list_matrices,
                                                                          contour_level=max_technical_load_caution,
