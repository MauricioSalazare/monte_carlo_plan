...
def plot_quantile_mixture_case(solution_quantile, ax):
    ax.plot(solution_quantile["max_q_95"], linestyle="--", color="r", linewidth=0.8, label=r"95 \%")
    ax.plot(solution_quantile["max_q_90"], linestyle="--", color="b", linewidth=0.8, label=r"90 \%",
            marker="o", markersize=1)
    ax.plot(solution_quantile["max_q_75"], linestyle="--", color="g", linewidth=0.8, label=r"75 \%")
    ax.plot(solution_quantile["max_q_50"], linestyle="-", color="r", linewidth=0.8, label=r"50 \%")
    ax.plot(solution_quantile["max_q_25"], linestyle="-.", color="g", linewidth=0.8, label=r"25 \%")
    ax.plot(solution_quantile["max_q_10"], linestyle="-.", color="b", linewidth=0.8, label=r"10 \%")
    ax.plot(solution_quantile["max_q_05"], linestyle="-.", color="r", linewidth=0.8, label=r"5 \%")
...
for ii, (ax_0, ax_1, ax_2, ax_3, ax_4, mixture_case) in enumerate(zip(ax_0_row,
    ...
    for data_mixture_ in data_mixture_case:
        coords_.append(get_coords_max_values(data_mixture_["max_q_90"]))
    ...
    for ax_fig, color_, data_mixture_, (x_max_, y_max_) in zip(ax_iter, colors_, data_mixture_case, coords_):
        plot_quantile_mixture_case(data_mixture_, ax=ax_fig)
...
def get_matrices_critical_quantiles(quant: str,
                                    all_mixtures: set,
                                    solutions_dict: dict,
    ...
    """
    Computes the max or min voltage of all nodes and all time steps for the all the mixture combinations and
    combinations of pv and load growth.

    Parameter:
    ----------
        quant: str: Quantile to compute. Can be ["50", "75", "90", "95"]
        all_mixtures: set: Set of tuples with the combinations of miture of type of day.
        ...
        process_max: bool: True if compute the maximum. False for the minimum.

    Return:
    -------
        list_matrices: list: Each item in the list is a (2-D) matrix for each one of the mixture combinations for one
            particular quantile specified by parameter quant.
    ...
    for mixture_case in all_mixtures:
        for i, pv in enumerate(pv_growth_percentiles):  # x == pv growth percentiles
            for j, load in enumerate(load_growth_percentiles):  # y == load growth percentiles
                case = (mixture_case, load, pv)
                if process_max:
                    matrix_voltages[i, j] = np.max(solutions_dict[case]["max_q_" + quant])
                else:
                    matrix_voltages[i, j] = np.min(solutions_dict[case]["min_q_" + quant])
        list_matrices.append(matrix_voltages.copy())
...
critical_quantile_min = ["75", "90", "95"]
titles_list = ["(a)", "(b)", "(c)"]
min_quant_span = [0.82, 0.85, 0.86]
...
for ii, ax in enumerate(ax_):  # Iterate through columns
    ...
    q_05_max, q_50_max, q_95_max = plot_contour_levels_irradiance_days(list_matrices,
                                                                       ...
                                                                       load_growth_percentiles=y,
                                                                       ax=ax)
    ax.fill_between(x, 0, q_05_max, color="green", alpha=0.2, zorder=0)
    ax.fill_between(x, q_05_max, q_95_max, color="orange", alpha=0.2, zorder=0)
    ax.fill_between(x, q_95_max, 1, color="red", alpha=0.2, zorder=0)
    ...
    ax.fill_between(x_new[idx], 0, y_new[idx], color="blue", alpha=0.2, zorder=0)
...
# ax.fill_between(x, 0, q_05_max, color="green", alpha=0.2)
# ax.fill_between(x,  q_05_max, q_95_max, color="orange", alpha=0.2)
# ax.fill_between(x,  q_95_max, 1, color="red", alpha=0.2)
